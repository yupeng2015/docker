...
PORTS_TO_DETECT='29666,29990,29903,29904,29981,29907,28995'
# for zsh compatiable
...
if [ ! -f "./app_deps/node-agent/magicstring" ]
then
...
mv /opt/swoole/node-agent/magicstring /opt/swoole/config/ >/tmp/na_installer.log 2>&1
...
foreach([$PORTS_TO_DETECT] as \$port){
    ...
    if (!@\$cli->connect("$ADMIN_ADDR", \$port)){
        echo "admin service $ADMIN_ADDR:\${port} is not accessible.\n";
        \$ret = 1;
    }else{
        \$cli->close();
    }
